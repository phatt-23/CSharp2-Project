    ...
    ) : Controller, IReservationsApi
{
    ...
    public async Task<IActionResult> GetResevationsAsync([FromQuery] ReservationQueryRequestDto request)
    {
        ...
        var paginatedReservations = paginationService.Paginate(reservations, request.PageNumber, request.PageSize);
    ...
    public async Task<IActionResult> CancelReservationAsync(int id)
    {
        try
        {
            ...
            var canceledReservation = await reservationService.CancelReservationAsync(reservation.Id);
